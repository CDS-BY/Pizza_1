	...
	const renderItems = (arr) => {
		...
		return arr.map(({id, name, price, imageUrl, ...props}) => (
			<li key={id} {...props} className="conten-wok__item item-content-wok">
				...
				<input className="item-content-wok__checkbox" type="checkbox" name="1"/>
			</li>
		))
	}
...
//===================================================================
// Костыль. Различия в названиях данных передаваемых и тех, которые используются для генерации объектов
	
	const name = wokName
	const price = wokPrice
	const weight = wokWeight
